...
const tribonacci = require('../lib/vanilla');
const letterScores = require('../lib/letterScores');

describe('App', () => {
  describe('tribonacci()', () => {
    it('should should have a tribonacci method', () => {
      assert.isFunction(tribonacci);
    });
    it('should return a tribonacci sequence', () => {
    ...
    it('should return [] if n===0', () => {
      assert.equal((tribonacci([0,9]),0),[])
      assert.equal((tribonacci([0,9,12]),0),[])
    });
    it('should return a length an array of n', () => {
      assert.deepEqual(tribonacci([1,1,1],4).length,4)
      assert.deepEqual(tribonacci([1,2,3],6).length,6)
    })
